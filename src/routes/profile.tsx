...
export const Route = createFileRoute("/profile")({
  head: () => ({
    meta: [
      { title: "Profile & Settings — FinWise" },
...
function SettingRow({
  ...
  return (
    <button
      onClick={onClick}
      className="flex w-full items-center gap-3 rounded-xl p-3 text-left transition hover:opacity-80 active:scale-[0.99]"
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <div className="text-sm font-medium" style={{ color: danger ? "var(--danger)" : "var(--foreground)" }}>
          {label}
        </div>
        {sub && (
    ...
    <div className="mx-auto flex max-w-2xl flex-col gap-6">
      ...
      <SettingsCard title="Notifications">
        <SettingRow
          ...
          sub="Budget alerts, goal reminders"
          iconColor="#EC4899"
        ...
        <SettingRow
          icon={Bell}
          label="Email digest"
          sub="Weekly summary email"
          iconColor="#F59E0B"
          iconBg="rgba(245,158,11,0.12)"
          right={<Toggle on={emailDigest} onToggle={() => setEmailDigest((n) => !n)} />}
          onClick={() => setEmailDigest((n) => !n)}
        />
      </SettingsCard>
